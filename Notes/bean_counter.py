...
# - Report on the page of all green beans
# Version 0.3
...
red_pixels = []        # where pixels are stores
green_pixels = []      # where pixels are stores
blue_pixels = []       # where pixels are stored
...
for y in range(jelly_bean_img.height):
    for x in range(jelly_bean_img.width):
        ...
        # Store its location somewhere 
            red_pixels.append((x,y))               # was outside of the "for x" loop 
...
for pixel_loc in blue_pixels: 
    blue_pixel_map.putpixel(pixel_loc, BLUE_PIXEL)         # Takes two arguments -> location and value of pixel
...
print(f"Blue Jelly Bean: {round(blue_pixel_percentage, 2)}%")
